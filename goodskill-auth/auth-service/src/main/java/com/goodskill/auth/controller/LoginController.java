...
public class LoginController {
    ...
    public Result<LoginTokenVO> login(@Valid @RequestBody UserDTO userDTO) {
        ...
        if (userAccount != null) {
            ...
            return Result.ok(buildLoginTokenVO(accessToken, refreshToken, accessExpireSeconds, userAccount));
        }
        return Result.fail("用户名或密码错误");
    }
    ...
    public Result<LoginTokenVO> refresh(@Valid @RequestBody RefreshTokenDTO request) {
        ...
        long accessExpireSeconds = authTokenFacade.getAccessTokenTimeout(accessToken);
        return Result.ok(buildLoginTokenVO(accessToken, newRefreshToken, accessExpireSeconds, user));
    }

    private LoginTokenVO buildLoginTokenVO(String accessToken, String refreshToken, long accessExpireSeconds, User user) {
        return LoginTokenVO.builder()
                ...
                .emailAddr(user != null ? user.getEmailAddr() : null)
                .build();
    }
}
