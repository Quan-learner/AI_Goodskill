...
const systemStats = ref([
  { label: '今日订单', value: '0', change: '0%', trend: 'up' },
  { label: '活跃用户', value: '0', change: '0%', trend: 'up' },
  { label: '商品总数', value: '0', change: '0%', trend: 'up' },
  { label: '待处理', value: '0', change: '0%', trend: 'down' }
])

const recentActivities = ref([])
const isRefreshing = ref(false)

const orderApi = createRequest('/api/order')
const authApi = createRequest('/api/auth')
const seckillApi = createRequest('/api/seckill')
...
const fetchDashboardData = async () => {
  ...
  try {
    ...
    try {
      const res = await orderApi.get('/admin/list', { params: { page: 1, size: 1, startTime: todayStr } })
      if (res.data && res.data.code === 0 && res.data.data) {
        todayOrdersCount = res.data.data.total
      }
    } catch (e) {
    ...
    let usersCount = 0
    try {
      const res = await authApi.get('/user/list', { params: { page: 1, size: 1 } })
      if (res.data && res.data.code === 0 && res.data.data) {
        usersCount = res.data.data.total
      }
    } catch (e) {
    ...
    let goodsCount = 0
    try {
      const res = await seckillApi.get('/goods/list', { params: { page: 1, size: 1 } })
      if (res.data && res.data.code === 0 && res.data.data) {
        goodsCount = res.data.data.total
      }
    } catch (e) {
    ...
    try {
      const res = await orderApi.get('/admin/list', { params: { page: 1, size: 1, status: 1 } })
      if (res.data && res.data.code === 0 && res.data.data) {
        pendingOrdersCount = res.data.data.total
      }
    } catch (e) {
      console.error('获取待支付订单失败:', e)
    }

    // 更新 systemStats
    systemStats.value[0].value = String(todayOrdersCount)
    systemStats.value[1].value = String(usersCount)
    systemStats.value[2].value = String(goodsCount)
    systemStats.value[3].value = String(pendingOrdersCount)
    
    systemStats.value[0].change = todayOrdersCount > 0 ? '+100%' : '0%'
    systemStats.value[1].change = usersCount > 0 ? '+100%' : '0%'
    systemStats.value[2].change = goodsCount > 0 ? '+100%' : '0%'
    systemStats.value[3].change = '实时'
    systemStats.value[3].trend = pendingOrdersCount > 0 ? 'up' : 'down'

    // 5. 最近动态
    try {
      const res = await orderApi.get('/admin/list', { params: { page: 1, size: 5 } })
      if (res.data && res.data.code === 0 && res.data.data && Array.isArray(res.data.data.records)) {
        recentActivities.value = res.data.data.records.map(order => {
          const phone = order.userPhone
          const maskedPhone = phone && phone.length >= 11 
          ...
          return {
            user: maskedPhone,
            action: action,
            target: order.goodsName || `秒杀商品 #${order.seckillId}`,
        ...
        <div class="dashboard-grid">
          ...
          <section class="activity-section fade-in-up" style="--delay: 0.3s">
            <div class="section-header">
              ...
              <button class="refresh-btn" @click="fetchDashboardData" :disabled="isRefreshing">
