...
This project is named **AI_Goodskill**, which is a simplified and AI-intelligent sandbox system rebuilt on top of the original `goodsKill` high-concurrency flash sale simulation demo.

It aims to construct an easy-to-learn, test, and demonstrate microservice concurrent simulation platform, focusing on three optimized core flash sale locking strategies. The project also integrates Spring AI features, allowing users to run simulation tasks, query statistics, and converse with a knowledge base through an AI assistant.

The overall architecture adopts Spring Boot 3.x and Spring Cloud. Efficient microservice communication is implemented via Dubbo 3.x and Feign, with Nacos serving as the service registration/discovery and configuration center. To keep the project focused and lightweight, it has been streamlined to highlight concurrent locking strategies and AI integration, and does not include database sharding configurations.

Since SpringCloudAlibaba doesn't officially support Dubbo 3.x yet, this project uses dubbo-spring-boot-starter for integration.
...
|--goodskill-seata                          ||Example of distributed transaction solution integrating nacos+dubbo+seata
...
Currently implements three core flash sale solutions, accessible through `SeckillMockController` test interfaces.

* **Solution 1: Synchronized lock implementation** (JVM-level synchronization lock, limited to single instance deployment)
* **Solution 2: Redisson distributed lock implementation** (Redis-based distributed lock with Watchdog auto-renewal, suitable for high-throughput distributed deployment)
* **Solution 3: ZooKeeper distributed lock** (ZooKeeper-based distributed lock with Apache Curator, strong consistency guarantee, suitable for high-consistency distributed deployment)

<details>
<summary>You can view flash sale results and analysis in web console, with logs similar to:</summary>

```text
2026-06-25 21:58:59.857  INFO [goodskill-web,...] o.s.w.controller.SeckillMockController   : Flash sale solution 2 (Redis distributed lock) start time: Wed Apr 14 21:58:59 CST 2021, seckillId: 1000
...
---------------------------------------------
ns         %     Task name
---------------------------------------------
3343433700  021%  Solution 1 (Synchronized lock)
3273092300  020%  Solution 2 (Redis distributed lock)
9189080600  059%  Solution 7 (ZooKeeper distributed lock)
```
...
* Start default services (nacos, redis, mysql, zookeeper, elasticsearch, seataServer) or use docker-compose[^2]:
...
* This is a high-concurrency flash sale simulation demo project based on Spring Cloud and Spring Boot.
