...
const mockUsers = [
  { username: 'test03', password: 'aa123456' }
]

// Track logged in user in memory, restored from localStorage if available
let currentUser = (() => {
  try {
    const userinfo = localStorage.getItem('userinfo')
    if (userinfo) {
      return JSON.parse(userinfo).username || 'test03'
    }
  } catch (e) {
    // Ignore
  }
  ...
  login: (data) => {
    if (USE_MOCK) {
      return new Promise((resolve) => {
        setTimeout(() => {
          const userExists = mockUsers.some(u => u.username === data.username && u.password === data.password)
          if (userExists) {
            currentUser = data.username
            resolve({
              code: 200,
              data: {
                ...mockData.login.success.data,
                userId: 1,
                username: data.username,
                mobile: '13800138000',
                avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=200&h=200&fit=crop',
                emailAddr: data.username + '@example.com'
              }
            })
          } else {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          if (mockUsers.some(u => u.username === data.username)) {
            ...
            mockUsers.push({ username: data.username, password: data.password })
            resolve({
              code: 200,
              message: '注册成功'
            })
        ...
        setTimeout(() => {
          resolve({
            ...
            data: {
              ...
              mobile: '13800138000',
              avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=200&h=200&fit=crop',
              emailAddr: currentUser + '@example.com'
            }
          })
        }, 300)
      })
