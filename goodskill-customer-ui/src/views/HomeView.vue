<template>
  <div class="min-h-screen bg-[var(--bg-primary)] transition-colors duration-300">
        ...
        <div class="flex flex-col lg:flex-row items-center gap-12">
          ...
          <div class="lg:w-1/2 text-center lg:text-left">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <button @click="scrollToProductList" class="btn-primary px-8 py-4 text-base font-semibold rounded-2xl flex items-center justify-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex gap-8 mt-12 justify-center lg:justify-start">
              <div class="text-center">
                <div class="text-2xl font-bold text-[var(--text-primary)] font-display">{{ activeUsers }}</div>
                <div class="text-sm text-[var(--text-muted)]">活跃用户</div>
              </div>
              <div class="w-px bg-[var(--border-color)]"></div>
              <div class="text-center">
                <div class="text-2xl font-bold text-[var(--text-primary)] font-display">{{ activeGoodsCount }}</div>
                <div class="text-sm text-[var(--text-muted)]">商品数量</div>
              </div>
              <div class="w-px bg-[var(--border-color)]"></div>
              <div class="text-center">
                <div class="text-2xl font-bold text-[var(--text-primary)] font-display">{{ positiveRatingRate }}</div>
              ...
              <div class="absolute -bottom-6 -left-6 glass-card p-4 rounded-2xl shadow-card animate-float">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div class="text-lg font-bold text-[var(--text-primary)] font-display">{{ todaySoldCount }} 件</div>
                  </div>
                </div>
              </div>
            </div>
    ...
    <!-- 商品列表 -->
    <section id="product-list-section" class="py-12">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Section Header -->
...
const activeUsers = computed(() => {
  ...
  return (10245 + likesCount * 5).toLocaleString()
})

const activeGoodsCount = computed(() => {
...
const positiveRatingRate = computed(() => {
  const likesCount = Number(localStorage.getItem('likes_count') || 0)
  const rate = 98.6 + likesCount * 0.05
  const finalRate = rate >= 100 ? 100 : rate
  return finalRate.toFixed(1) + '%'
})

const todaySoldCount = computed(() => {
  const soldCount = Number(localStorage.getItem('today_sold_count') || 2847)
  return soldCount.toLocaleString()
})
    ...
    if (response.code === 0 || response.code === 200) {
      if (response.data && response.data.records) {
        const transformedGoods = response.data.records.map((goods) => {
          const rawImg = goods.goodsImg || goods.photoUrl
          const goodsName = goods.goodsName || goods.name
...
onMounted(() => {
  ...
  fetchGoodsList()
  fetchUserInfo()
  if (route.query.scroll === 'products') {
    setTimeout(() => {
      scrollToProductList()
    }, 300)
  }
