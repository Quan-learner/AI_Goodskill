            ...
            <div class="glass-card rounded-3xl p-8">
              ...
              <div class="space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <!-- 账户余额 -->
                  <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-[var(--text-secondary)] mb-2">账户余额</label>
                    <div
                      class="w-full px-4 py-3 rounded-xl bg-black/5 dark:bg-white/5 text-[var(--text-primary)] font-mono font-semibold text-lg flex items-center gap-2"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      ¥{{ Number(user.balance).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }} 元
                    </div>
                  </div>
                </div>
              </div>
...
const fetchUserInfo = async () => {
  try {
    ...
    if (response.code === 0 || response.code === 200) {
      ...
      // 检查本地是否有最新的头像缓存（Base64 缓存优先），防止被后端返回的空或旧头像覆写
      const localCache = localStorage.getItem('local_avatar_cache')
      const userinfoStr = localStorage.getItem('userinfo')
      let localAvatar = localCache
      if (!localAvatar && userinfoStr) {
        try {
      ...
      // 同步到全局 store，以便其他页面（如首页、订单页等）也看到最新头像
      updateUserInfo(data)
    }
  } catch (error) {
        ...
        canvas.toBlob(
          (blob) => {
            if (blob) {
              ...
              reject(new Error('Canvas 压缩图片生成失败'))
            }
          },
          file.type,
          quality
...
const handleAvatarChange = async (event) => {
  ...
  try {
    ...
    // 2. 尝试上传头像到 common 服务 (MinIO)
    const uploadResponse = await api.uploadAvatar(compressedFile)
    if (uploadResponse.code === 0 || uploadResponse.code === 200) {
      ...
      if (updateResponse.code === 0 || updateResponse.code === 200) {
        ...
        throw new Error(updateResponse.message || updateResponse.msg || '保存头像至数据库失败')
      }
    } else {
      throw new Error(uploadResponse.message || uploadResponse.msg || '头像文件上传失败')
    }
  } catch (error) {
