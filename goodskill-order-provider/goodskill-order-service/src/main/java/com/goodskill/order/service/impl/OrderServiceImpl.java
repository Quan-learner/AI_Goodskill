...
public class OrderServiceImpl {
    ...
    public Boolean confirmReceipt(String orderId, String userId) {
        Optional<Order> optionalOrder = orderRepository.findById(orderId);
        if (optionalOrder.isPresent()) {
            Order order = optionalOrder.get();
            // 验证订单是否属于当前用户
            if (order.getUserId() == null || !order.getUserId().equals(userId)) {
                log.warn("确认收货失败: 订单不属于当前用户, orderId={}, userId={}", orderId, userId);
                return false;
            }
            // 验证订单状态是否可以确认收货（已支付状态可以确认收货）
            if (order.getStatus() == null || !order.getStatus().equals((byte) 2)) {
                log.warn("确认收货失败: 订单状态不允许确认收货, orderId={}, status={}", orderId, order.getStatus());
                return false;
            }
            // 更新订单状态为已收货 (状态 4)
            order.setStatus((byte) 4);
            order.setStateDesc("已收货");
            orderRepository.save(order);
