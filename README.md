...
本项目命名为 **AI_Goodskill**，是在原 `goodsKill` 高并发秒杀模拟演练项目的基础上进行简化和 AI 智能化改造的沙箱系统。

本项目旨在构建一个易于学习、测试 and 演示的微服务并发模拟平台，重点保留并优化了三种核心秒杀锁策略。同时集成了 Spring AI 功能，允许用户通过智能 AI 助手完成秒杀模拟、统计分析以及相关的知识库问答。

项目架构整体采用 Spring Boot 3.x + Spring Cloud 体系，使用 Dubbo 3.x 与 Feign 完成微服务间的高效接口调用，服务注册发现以及配置中心采用 Nacos。为了方便学习和专注于并发锁机制，本项目已进行了精简，专注于展示核心的多锁秒杀策略与 AI 交互，并不包含任何数据库分库分表操作。

由于SpringCloudAlibaba官方暂未支持Dubbo 3.x，本项目采用dubbo-spring-boot-starter集成。
...
|--goodskill-seata                          ||集成nacos+dubbo+seata的分布式事务解决方案示例
...
目前仅保留三种核心的秒杀方案，通过`SeckillMockController`提供测试接口

* **场景一：Sychronized同步锁实现** (JVM级同步锁，限制单体应用/单机部署)
* **场景二：Redisson分布式锁实现** (基于Redis的分布式锁，带看门狗续期自动延长锁有效期，适用于高吞吐分布式场景)
* **场景三：ZooKeeper分布式锁** (基于ZooKeeper节点特性的分布式锁，强一致性保障，适用于高一致性分布式场景)

<details>
<summary>可在控制台查看秒杀耗时与分析结果，打印信息类似：</summary>

```text
2026-06-25 21:58:59.857  INFO [goodskill-web,...] o.s.w.controller.SeckillMockController   : 秒杀场景二(redis分布式锁实现)开始时间：Wed Apr 14 21:58:59 CST 2021,秒杀id：1000
...
---------------------------------------------
ns         %     Task name
---------------------------------------------
3343433700  021%  秒杀场景一(sychronized同步锁实现)
3273092300  020%  秒杀场景二(redis分布式锁实现)
9189080600  059%  秒杀场景七(zookeeper分布式锁)
```
...
* 默认端口启动nacos、redis、mysql、zookeeper、elasticsearch、seataServer，或者使用docker-compose[^2]命令：
...
管理端前端项目位于 `goodskill-ui` 目录下，基于Vue 3 + Vite构建。
...
C端用户前端项目位于 `goodskill-customer-ui` 目录下，是一个面向消费者的秒杀商城前端应用。
...
开发服务器启动后，可通过以下地址访问前端项目：`http://localhost:5174`
...
* **用户认证**：登录、注册、退出登录
* **首页展示**：商品分类、热门秒杀商品展示、搜索功能
* **商品详情**：商品信息展示、倒计时秒杀、立即抢购
...
| 登录页面 | 工作台首页 |
|:---:|:---:|
| <img src="docs/shortcut/image-2026-01-29-18-00-41-208.png" width="400"> | <img src="docs/shortcut/image-2026-01-30-00-17-27-581.png" width="400"> |

| 用户管理 | 商品管理 | 秒杀模拟 |
|:---:|:---:|:---:|
| <img src="docs/shortcut/image-2026-01-29-18-01-31-190.png" width="260"> | <img src="docs/shortcut/image-2026-01-29-18-01-52-357.png" width="260"> | <img src="docs/shortcut/image-2026-01-29-18-02-22-212.png" width="260"> |

### C端用户前端页面

| 商城首页 | 商品详情页 | 订单详情页 |
|:---:|:---:|:---:|
| <img src="docs/shortcut/img_3.png" width="260"> | <img src="docs/shortcut/img_4.png" width="260"> | <img src="docs/shortcut/img_5.png" width="260"> |
...
* 这是一个基于 Spring Cloud + Spring Boot 架构的高并发秒杀模拟演练项目。
